...
public interface ClassGroupRepo extends JpaRepository<ClasseGroup, Integer> {
    ...
    @Query(value = "SELECT cp.classe_id, cg.class_room_name, p.identification_id, p.username, p.email, p.gender, p.age FROM classe_professeur cp INNER JOIN person p ON p.class_group_id = cp.classe_id INNER JOIN classe_group cg WHERE cp.professeur_id =:id AND dtype = 'ELEVE' ", nativeQuery = true)
